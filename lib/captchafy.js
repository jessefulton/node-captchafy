				...
				if (!!text && !!fontSize && (_jQuery(this.parentNode).is(':visible'))) {
					...
					for (var i=0; i<words.length; i++) {
						...
						try {
							...
							captcha.add(ex.text.wavy, {"text": text, "size": fontSize, "fillStyle": color, "font": fontface });
							if (word.length > 2) {
								captcha.add(ex.noise.blob, {"fillStyle": color, "size": fontSize, "h": captcha.fontSize*.75, "w": (captcha.textWidth/1.5) /*, "x": (captcha.textWidth/3.0), "y":0*/ });
