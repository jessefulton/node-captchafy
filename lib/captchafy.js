...
var captchafy = function(canvas) {
	...
	this.crop = function() {
		...
		if (!hasPixel) {
			//console.log('no pixels for ' + this.text);
		}
...
var ex = {
	...
	"captchafyText": function(_jQuery, _el, callback) {
		...
		try {	
			getTextNodesIn(_el).each(function() {
				...
				console.log(_jQuery(this.parentNode).css("font-size"));
				...
				if (!!text && !!fontSize && (_jQuery(this.parentNode).is(':visible'))) {
					...
					var color = _jQuery(this.parentNode).css("color");
					
					//TODO: implement
					var fontface = "Times";
					try {
						fontface = _jQuery(this.parentNode).css("font-family").split(',')[0];
					}
					catch(errrrrrrrrrr) {}
					
					var fullFont = _jQuery(this.parentNode).css("font");
					...
					for (var i=0; i<words.length; i++) {
						...
						if (!word) { continue; }
						if (word.length < 3) {
							replacements.push("<span>" + word + "</span>");
							continue;
						}
						
						evenOdd = !evenOdd;
						//let's just do every other word
						if (evenOdd) { //words.length > 1 && i%2==0) {
							replacements.push("<span>" + word + "</span>");
							continue;
						}
						...
						try {
							...
							captcha.init(word, fontSize, fontface, fontSize/5);
			
							//captcha.add(ex.text.wavy, {"text": word, "size": fontSize, "fillStyle": color, "font": fontface });
							captcha.add(ex.text.wavy, {"text": word, "size": fontSize, "fillStyle": color, "fontFace": fontface, "font": fullFont });
							if (word.length > 2) {
								captcha.add(ex.noise.blob, {"fillStyle": color, "size": fontSize, "h": fontSize*.75, "w": (captcha.textWidth/1.5) /*, "x": (captcha.textWidth/3.0), "y":0*/ });
								//captcha.add(ex.noise.blob, {"fillStyle"	: color, "h": fontSize, "w": (fontSize * 1.5)})
							};

							captcha.render();
							captcha.crop();
							
							//console.log(word + ": " + captcha.canvas.height + 'x' + captcha.canvas.width);


							//replacements.push('<img src="' + captcha.toDataURL() + '" /> ');
							
							replacements.push('<span style="display:inline-block;width:' + captcha.canvas.width + 'px;height:' + captcha.canvas.height + 'px;background-image:url(\'' + captcha.toDataURL() + '\');overflow:hidden;padding:0;margin:0;"></span>');
	...
	"noise": {
		...
		var w = (options.w ? options.w : canvas.width/5)/2	
	...
	"text": {
		...
		"wavy": function(canvas, options, cb) {
			...
			var text = options.text ? options.text : "";
			var fontSize = options.size ? options.size : 64;
			var fontFace = options.fontFace ? options.fontFace : "Times";
			var fillStyle = options.fillStyle ? options.fillStyle : "black";
		
			var font = options.font ? options.font : fontSize + "px " + fontFace;
		
			context.save();
		
			context.font = font; //fontSize + "px " + fontFace;
			context.fillStyle = fillStyle;
			...
			var distortionAmount = Math.min(.5, (fontSize/64));
